...
pub fn convert_to_double_layer(input_path: &Path, output_path: &Path) -> Result<(), String> {
    ...
    if img.width() != img.height() * 2 {
        return Err("Input image is not a single-layer skin (width must be twice the height).".to_string());
    }
    ...
    let right_leg_top_front = img.view(right_leg_top_front_area.0, right_leg_top_front_area.1, right_leg_top_front_area.2 - right_leg_top_front_area.0, right_leg_top_front_area.3 - right_leg_top_front_area.1).to_image();
    let right_leg_buttom = img.view(right_leg_buttom_area.0, right_leg_buttom_area.1, right_leg_buttom_area.2 - right_leg_buttom_area.0, right_leg_buttom_area.3 - right_leg_buttom_area.1).to_image();
    let right_leg_inside = img.view(right_leg_inside_area.0, right_leg_inside_area.1, right_leg_inside_area.2 - right_leg_inside_area.0, right_leg_inside_area.3 - right_leg_inside_area.1).to_image();
    let right_leg_back = img.view(right_leg_back_area.0, right_leg_back_area.1, right_leg_back_area.2 - right_leg_back_area.0, right_leg_back_area.3 - right_leg_back_area.1).to_image();
    let right_leg_outside = img.view(right_leg_outside_area.0, right_leg_outside_area.1, right_leg_outside_area.2 - right_leg_outside_area.0, right_leg_outside_area.3 - right_leg_outside_area.1).to_image();

    // Crop right arm parts
    let right_arm_top = img.view(right_arm_top_area.0, right_arm_top_area.1, right_arm_top_area.2 - right_arm_top_area.0, right_arm_top_area.3 - right_arm_top_area.1).to_image();
    let right_arm_front = img.view(right_arm_front_area.0, right_arm_front_area.1, right_arm_front_area.2 - right_arm_front_area.0, right_arm_front_area.3 - right_arm_front_area.1).to_image();
    let right_arm_buttom = img.view(right_arm_buttom_area.0, right_arm_buttom_area.1, right_arm_buttom_area.2 - right_arm_buttom_area.0, right_arm_buttom_area.3 - right_arm_buttom_area.1).to_image();
    let right_arm_inside = img.view(right_arm_inside_area.0, right_arm_inside_area.1, right_arm_inside_area.2 - right_arm_inside_area.0, right_arm_inside_area.3 - right_arm_inside_area.1).to_image();
    let right_arm_back = img.view(right_arm_back_area.0, right_arm_back_area.1, right_arm_back_area.2 - right_arm_back_area.0, right_arm_back_area.3 - right_arm_back_area.1).to_image();
    let right_arm_outside = img.view(right_arm_outside_area.0, right_arm_outside_area.1, right_arm_outside_area.2 - right_arm_outside_area.0, right_arm_outside_area.3 - right_arm_outside_area.1).to_image();
    ...
    imageops::overlay(&mut output_img, &left_arm_outside, left_arm_outside_area.0 as i64, left_arm_outside_area.1 as i64);
    imageops::overlay(&mut output_img, &left_arm_top, left_arm_top_area.0 as i64, left_arm_top_area.1 as i64);
    imageops::overlay(&mut output_img, &left_arm_front, left_arm_front_area.0 as i64, left_arm_front_area.1 as i64);
    imageops::overlay(&mut output_img, &left_arm_buttom, left_arm_buttom_area.0 as i64, left_arm_buttom_area.1 as i64);
    imageops::overlay(&mut output_img, &left_arm_inside, left_arm_inside_area.0 as i64, left_arm_inside_area.1 as i64);
    imageops::overlay(&mut output_img, &left_arm_back, left_arm_back_area.0 as i64, left_arm_back_area.1 as i64);

    // Paste left leg parts
    imageops::overlay(&mut output_img, &left_leg_top_front, left_leg_top_front_area.0 as i64, left_leg_top_front_area.1 as i64);
    imageops::overlay(&mut output_img, &left_leg_buttom, left_leg_buttom_area.0 as i64, left_leg_buttom_area.1 as i64);
    imageops::overlay(&mut output_img, &left_leg_inside, left_leg_inside_area.0 as i64, left_leg_inside_area.1 as i64);
    imageops::overlay(&mut output_img, &left_leg_back, left_leg_back_area.0 as i64, left_leg_back_area.1 as i64);
    imageops::overlay(&mut output_img, &left_leg_outside, left_leg_outside_area.0 as i64, left_leg_outside_area.1 as i64);
